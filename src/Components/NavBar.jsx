    ...
    <header className="w-full bg-white shadow-md px-6 py-4 md:py-6 fixed top-0 left-0 right-0 z-50">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <nav className="hidden md:flex space-x-6">
          ...
          <Link href="/enterprise" className="hover:text-orange-500">Enterprise</Link>
        </nav>

        {/* CTA Buttons */}
        <div className="hidden md:flex space-x-4">
          <button className="bg-orange-500 text-white px-4 py-2 rounded-lg">Get a demo</button>
          <button className="border border-orange-500 text-orange-500 px-4 py-2 rounded-lg">Contact</button>
        </div>
      ...
      {/* Mobile Sidebar */}
      {isOpen && (
        <div className="md:hidden fixed top-0 left-0 w-3/4 h-full bg-white shadow-lg p-6">
...
const Dropdown = ({ title, mobile }) => (
  <div className={`relative ${mobile ? 'w-full' : ''}`}>
    <button className="hover:text-orange-500 flex items-center justify-between w-full">
